...
def train():
    ...
    dataset = GDELTDataset(lookback=lookback, horizon=1, step=1)
    ...
    }, f"checkpoints/TSMixer_isr_{ff_dim=}_{lookback=}_{num_blocks=}.pt")

    return valid_loss

if __name__ == "__main__":
    print(pt.accelerator.current_accelerator())
    # storage_name = f"sqlite:///TSMixer_study.db"
    #no-name-16720e9a-65e4-4bc8-92db-19b812c1f122
    train()
    # study = optuna.create_study(study_name="no-name-16720e9a-65e4-4bc8-92db-19b812c1f122", storage=storage_name, load_if_exists=True)
