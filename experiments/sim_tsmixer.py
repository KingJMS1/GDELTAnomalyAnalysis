...
def objective(trial: optuna.Trial):
    ...
    dataset = SimDataset(lookback=10, horizon=1, step=1)
    ...
    model = TSMixer(10, 1, 100, dataset.num_series, num_blocks=3, ff_dim=24).to(device)
    ...
    for epoch in tqdm_iter:
        ...
        if epoch % 100 == 0:
            ...
            }, f"checkpoints/TSMixer_sim/TSMixer_sim_ff_dim=24_lookback=10_num_blocks=3_{epoch=}.pt")
        
    return valid_loss

if __name__ == "__main__":
    print(pt.accelerator.current_accelerator())
    # storage_name = f"sqlite:///studies/TSMixer_small.db"
    #no-name-16720e9a-65e4-4bc8-92db-19b812c1f122
    objective(None)
    # study = optuna.create_study(study_name="TSMixer_small", storage=storage_name, load_if_exists=True)
